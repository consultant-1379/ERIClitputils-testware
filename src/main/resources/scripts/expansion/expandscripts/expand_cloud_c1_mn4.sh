...
for (( i=3; i<4; i++ )); do
    ...
    # BRIDGE ETH0
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if0 -t eth -o device_name=eth0 macaddress="${node_expansion_eth0_mac[$i]}" bridge='br0'
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/br0 -t bridge -o device_name=br0 ipaddress="${node_expansion_ip[$i]}" ipv6address="${node_expansion_ipv6_00[$i]}" forwarding_delay=4 network_name='mgmt'

    #litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if0 -t eth -o device_name=eth0 macaddress="${node_expansion_eth0_mac[$i]}" ipaddress="${node_expansion_ip[$i]}" ipv6address="${node_expansion_ipv6_00[$i]}" forwarding_delay=4 network_name='mgmt'

    # HEARTBEAT NETWORK SETUP
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if7 -t eth -o device_name=eth7 macaddress="${node_expansion_eth7_mac[$i]}" network_name=hb1
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if8 -t eth -o device_name=eth8 macaddress="${node_expansion_eth8_mac[$i]}" network_name=hb2
    
    # TRAFFIC NETWORKS
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if4 -t eth -o device_name=eth4 macaddress="${node_expansion_eth4_mac[$i]}" network_name='traffic1' ipaddress="${node_expansion_ip_2[$i]}"
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if5 -t eth -o device_name=eth5 macaddress="${node_expansion_eth5_mac[$i]}" network_name='traffic2' ipaddress="${node_expansion_ip_3[$i]}"

    #litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/br7 -t bridge -o device_name=br7 forwarding_delay=4 network_name=traffic4_v6
    #litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if7 -t eth -o bridge=br7 device_name=eth7 macaddress="${node_expansion_eth7_mac[$i]}"
    ...
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/br6 -t bridge -o device_name='br6' forwarding_delay=4 network_name=dhcp_network ipaddress="${dhcp_expansion_ip_1[$i]}"

    # BRIDGED NETWORK
    #litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if7 -t eth -o device_name=eth7 macaddress="${node_expansion_eth7_mac[$i]}" bridge='br7'
    #litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/br7 -t bridge -o device_name=br7 forwarding_delay=4 network_name='bridged'
